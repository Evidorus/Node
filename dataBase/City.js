...
const citiesModel = mongoose.model('cities', censusSchema)

// citiesModel.deleteMany({}).then(() => {citiesModel.create(
...
.group(
    {
    _id: '$department' , population: {$sum: '$population'}, moyenne: {$avg: '$population'}, nbVille: {$sum: 1}
    }
)
// .sort({_id: 1})
.sort({population: 1})
